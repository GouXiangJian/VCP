...
$jumpMarkDuplicates $software->{GATK} --java-options "-Dsamjdk.compression_level=5 -Xms4000m" MarkDuplicates --INPUT $outputDir/SortSamOut/$germplasm.sort.bam --OUTPUT $outputDir/MarkDuplicatesOut/$germplasm.sort.mark.bam --METRICS_FILE $outputDir/MarkDuplicatesOut/$germplasm.metrics --VALIDATION_STRINGENCY SILENT --OPTICAL_DUPLICATE_PIXEL_DISTANCE 2500 --ASSUME_SORT_ORDER coordinate --READ_NAME_REGEX null
